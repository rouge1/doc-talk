...
export default function Library() {
  ...
  return (
    <div className="rise">
      ...
      {lib.data && lib.data.documents.length === 0 && (
        <div className="empty">No documents yet — run <span className="mono">doctalk ingest &lt;file&gt;</span>.</div>
      )}
      ...
      {lib.data && lib.data.images > 0 && (
        <p className="muted" style={{ marginTop: "1.6rem" }}>
          + {lib.data.images} image(s) in the gallery.
        </p>
      )}

      <div className="rule-head">
        <h2>Synthesis wiki</h2>
      </div>
      <p className="muted">
        An LLM-authored, compounding knowledge base distilled from these sources —{" "}
        <Link to="/wiki">enter the wiki →</Link>
      </p>
    </div>
  );
}
